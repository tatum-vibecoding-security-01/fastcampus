...
- signals: 데이터에서 읽히는 핵심 신호 정확히 3개. 각 신호는 객체이며 title(짧은 제목)·detail(수치나 구체적 근거를 포함한 설명)·tone(positive/neutral/caution)을 모두 채웁니다.
- summary: 데이터 기반 분석가 톤의 종합 소견 2~4문장.
반드시 record_analysis 도구를 호출해 지정된 구조로만 결과를 제출하세요.`;
}
